...
var reviewedInPlaceByteWrites = map[string]reviewedInPlaceByteWrite{
	"internal/home/client.go":                               {1, "zeroes a private byte copy returned by redis StringCmd.Bytes after JSON decoding; no no-copy GJSON result is created"},
	"internal/pluginstore/auth.go":                          {1, "zeroes a locally allocated Basic auth credential after base64 encoding; it never contains a payload or no-copy GJSON result"},
