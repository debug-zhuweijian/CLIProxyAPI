...
func TestAntigravityReasoningReplayUnrelatedEvictionDoesNotBlockAbsentSnapshot(t *testing.T) {
	...
	antigravityReasoningReplayMu.Lock()
	liveEntry := antigravityReasoningReplayEntries[antigravityReasoningReplayCacheKey(model, "older-live-entry")]
	absentKey := antigravityReasoningReplayCacheKey(model, "untouched-absent-session")
	absentEntry := antigravityReasoningReplayEntries[absentKey]
	absentEntry.Timestamp = liveEntry.Timestamp
	antigravityReasoningReplayEntries[absentKey] = absentEntry
	evictOldestAntigravityReasoningReplayEntries(1)
	antigravityReasoningReplayMu.Unlock()
