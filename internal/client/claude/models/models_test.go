...
func TestResolveClaudeModelIDPrefixStrict(t *testing.T) {
	encoded := EnsureClaudeModelIDPrefix("glm-5.2[1m]")
	if got, err := ResolveClaudeModelIDPrefixStrict(encoded); err != nil || got != "glm-5.2[1m]" {
		t.Fatalf("strict round trip = %q, %v; want glm-5.2[1m], nil", got, err)
	}
	...
	for _, invalid := range []string{
		...
		if _, err := ResolveClaudeModelIDPrefixStrict(invalid); err == nil {
			t.Fatalf("ResolveClaudeModelIDPrefixStrict(%q) accepted an invalid encoded model", invalid)
		}
	}
}
