...
func TestManagementConfigProjectionRoutesRegistered(t *testing.T) {
	t.Setenv("MANAGEMENT_PASSWORD", "test-management-key")
	...
	routes := []struct {
		...
		{http.MethodGet, "/v0/management/config/functional"},
		{http.MethodPut, "/v0/management/config/functional"},
		{http.MethodGet, "/v0/management/config/static-auth"},
		{http.MethodGet, "/v0/management/config/static-auth/summary"},
		{http.MethodPut, "/v0/management/config/static-auth"},
	}
	for index, route := range routes {
		req := httptest.NewRequest(route.method, route.path, nil)
		req.RemoteAddr = fmt.Sprintf("192.0.2.%d:1234", index+10)
		rr := httptest.NewRecorder()
		server.engine.ServeHTTP(rr, req)
		if rr.Code != http.StatusUnauthorized {
			t.Fatalf("%s %s status = %d, want %d; body=%s", route.method, route.path, rr.Code, http.StatusUnauthorized, rr.Body.String())
		}
	}
	...
	for _, path := range []string{
		...
		"/v0/management/config/static-auth/summary",
	} {
		...
		if strings.Contains(path, "/static-auth") && rr.Header().Get("Cache-Control") != "no-store" {
			t.Fatalf("GET %s Cache-Control = %q, want no-store", path, rr.Header().Get("Cache-Control"))
		}
	}
}
