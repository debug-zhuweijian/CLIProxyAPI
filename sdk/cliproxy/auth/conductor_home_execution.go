...
func (m *Manager) executeHome(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, countTokens bool) (cliproxyexecutor.Response, error) {
	...
	for homeAuthCount := 1; ; homeAuthCount++ {
		...
		for _, upstreamModel := range models {
			...
			execOpts, errExecute = m.sealModelExecution(preparedAuth, selection.Provider, selection.Executor, execReq, execOpts)
			if errExecute != nil {
				releaseAttempt()
				selection.End("model_policy_rejected")
			...
			if countTokens {
				response, errExecute = m.countApproved(execCtx, selection.Executor, preparedAuth, selection.Provider, execReq, execOpts)
			} else {
				response, errExecute = m.executeApproved(execCtx, selection.Executor, preparedAuth, selection.Provider, execReq, execOpts)
			}
