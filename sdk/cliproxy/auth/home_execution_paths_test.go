...
func (e *homeExecutionExecutor) CountTokens(context.Context, *Auth, cliproxyexecutor.Request, cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	e.countCalls.Add(1)
...
type homeModelPolicyDispatcher struct{}

func (homeModelPolicyDispatcher) HeartbeatOK() bool { return true }

func (homeModelPolicyDispatcher) RPopAuth(context.Context, string, string, http.Header, int) ([]byte, error) {
	return json.Marshal(homeAuthDispatchResponse{
		Model: "glm-5.2",
		Auth:  Auth{ID: "home-policy-auth", Provider: "home-execution", Status: StatusActive},
	})
}

func (homeModelPolicyDispatcher) AbortAmbiguousDispatch() {}

func TestHomeNonStreamingExecutionRevalidatesModelPolicyAfterInterceptor(t *testing.T) {
	tests := []struct {
		name string
		...
		{
			name: "execute",
			run: func(manager *Manager, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) error {
				_, err := manager.Execute(context.Background(), []string{"home-execution"}, req, opts)
				return err
			},
		},
		{
			name: "count_tokens",
			run: func(manager *Manager, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) error {
				_, err := manager.ExecuteCount(context.Background(), []string{"home-execution"}, req, opts)
				return err
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cfg.ModelPolicy = internalconfig.ModelPolicyConfig{
				...
				CatalogAllowlist: []string{"glm-5.2[1m]"},
			...
			manager.PublishHomeDispatch(homeModelPolicyDispatcher{}, executionregistry.New(), 1)
			executor := &homeExecutionExecutor{}
			manager.RegisterExecutor(executor)
			...
			opts := cliproxyexecutor.Options{
				...
				RequestAfterAuthInterceptor: func(context.Context, cliproxyexecutor.RequestAfterAuthInterceptRequest) cliproxyexecutor.RequestAfterAuthInterceptResponse {
					return cliproxyexecutor.RequestAfterAuthInterceptResponse{Body: []byte(`{"model":"gpt-5.6-sol"}`)}
				},
			}
			req := cliproxyexecutor.Request{
				Model:   "glm-5.2[1m]",
				Payload: []byte(`{"model":"glm-5.2"}`),
			}
			if err := tt.run(manager, req, opts); err == nil {
				t.Fatal("Home execution accepted a model-changing post-auth interceptor")
			}
			if executor.executeCalls.Load() != 0 || executor.countCalls.Load() != 0 {
				t.Fatalf("rejected request reached executor: execute=%d count=%d", executor.executeCalls.Load(), executor.countCalls.Load())
			}
		})
	}
}
